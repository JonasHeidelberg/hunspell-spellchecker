...
Dictionary.prototype.load = function (obj) {
    for (var i in obj) {
        if (i == "compoundRuleStrings") {
            this.compoundRules = obj[i].map(exp => new RegExp(exp, "i"));
            // compoundRules not possible to stringify since they are regexes, so restore from the original strings instead
        } 
        this[i] = obj[i];
    }
};
...
Dictionary.prototype.toJSON = function(dictionary) {
    return {
        ...
        compoundRuleStrings: this.compoundRuleStrings, // compoundRules not possible to stringify since they are regexes, so store the original strings instead
...
Dictionary.prototype.parse = function(dictionary) {
    ...
    for (var i = 0, _len = this.compoundRules.length; i < _len; i++) {
        ...
        this.compoundRuleStrings[i] = expressionText;
    }
};
